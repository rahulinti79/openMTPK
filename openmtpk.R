...
`sum` = function(num1, num2, .copy = FALSE)
{
  num1 = as.integer(num1);
  
  if(length(num1) > 1) {
    warning("using only the first element of num1");
  };
  
  num2 = as.integer(num2);
  
  if(length(num2) > 1) {
    warning("using only the first element of num2");
  };
  
  ;.Call('R_swig_sum', num1, num2, as.logical(.copy), PACKAGE='openmtpk');
  
}

attr(`sum`, 'returnType') = 'integer'
attr(`sum`, "inputTypes") = c('integer', 'integer')
class(`sum`) = c("SWIGFunction", class('sum'))
